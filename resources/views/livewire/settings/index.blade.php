<div>
    ...
    <form wire:submit='submit' class="flex flex-col">
        ...
        <div>General configuration for your Hostly instance.</div>
            ...
            <div class="flex flex-wrap items-end gap-2">
                <div class="flex gap-2 md:flex-row flex-col w-full">
                    <x-forms.input id="fqdn" label="Instance's Domain"
                        ...
                        placeholder="https://hostly.yourdomain.com" />
                    <x-forms.input id="instance_name" label="Instance's Name" placeholder="Hostly" />
                    <div class="w-full" x-data="{
                        ...
                        <div class="flex items-center mb-1">
                            <label for="instance_timezone">Instance
                                Timezone</label>
                            <x-helper class="ml-2"
                                helper="Timezone for the Hostly instance. This is used for the update check and automatic update frequency." />
                        </div>
                        <div class="relative">
                ...
                <div class="flex gap-2 md:flex-row flex-col w-full">
                    <x-forms.input id="public_ipv4" type="password" label="Instance's IPv4"
                        helper="Enter the IPv4 address of the instance.<br><br>It is useful if you have several IPv4 addresses and Hostly could not detect the correct one."
                        placeholder="1.2.3.4" autocomplete="new-password" />
                    <x-forms.input id="public_ipv6" type="password" label="Instance's IPv6"
                        helper="Enter the IPv6 address of the instance.<br><br>It is useful if you have several IPv6 addresses and Hostly could not detect the correct one."
        ...
        <div class="text-right md:w-96">
            @if (!is_null(config('constants.hostly.autoupdate', null)))
                <div class="text-right md:w-96">
                    <x-forms.checkbox instantSave helper="AUTOUPDATE is set in .env file, you need to modify it there."
                        disabled checked="{{ config('constants.hostly.autoupdate') }}" label="Auto Update Enabled" />
                </div>
            @else
        ...
        <div class="flex flex-col gap-2">
            <div class="flex items-end gap-2">
                <x-forms.input required id="update_check_frequency" label="Update Check Frequency"
                    placeholder="0 * * * *"
                    helper="Cron expression for update check frequency (check for new Hostly versions and pull new Service Templates from CDN).<br>You can use every_minute, hourly, daily, weekly, monthly, yearly.<br><br>Default is every hour." />
                <x-forms.button wire:click='checkManually'>Check Manually</x-forms.button>
            </div>

            @if (is_null(config('constants.hostly.autoupdate', null)) && $is_auto_update_enabled)
                <x-forms.input required id="auto_update_frequency" label="Auto Update Frequency" placeholder="0 0 * * *"
                    helper="Cron expression for auto update frequency (automatically update hostly).<br>You can use every_minute, hourly, daily, weekly, monthly, yearly.<br><br>Default is every day at 00:00" />
            @endif
        </div>
