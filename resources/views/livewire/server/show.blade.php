<div>
    <x-slot:title>
        {{ data_get_str($server, 'name')->limit(10) }} > General | Hostly
    </x-slot>
                    ...
                    @if ($server->id === 0)
                        <x-modal-confirmation title="Confirm Server Settings Change?" buttonTitle="Save"
                            submitAction="submit" :actions="[
                                'If you missconfigure the server, you could lose a lot of functionalities of Hostly.',
                        ...
                        @if (!$server->isLocalhost())
                            ...
                            @if (!$server->isBuildServer() && !$server->settings->is_cloudflare_tunnel)
                                ...
                                <div class="pb-4">Read the docs <a class='underline dark:text-white'
                                        href='https://hostly.khulnasoft.com/docs/knowledge-base/docker/swarm'
                                        target='_blank'>here</a>.
                                </div>
                                <div class="w-96">
                                    @if ($server->settings->is_swarm_worker)
                                        <x-forms.checkbox disabled instantSave type="checkbox" id="isSwarmManager"
                                            helper="For more information, please read the documentation <a class='dark:text-white' href='https://hostly.khulnasoft.com/docs/knowledge-base/docker/swarm' target='_blank'>here</a>."
                ...
                <form wire:submit.prevent='submit'>
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        @if ($server->isSentinelEnabled())
                            ...
                            <x-forms.input id="sentinelCustomUrl" required label="Hostly URL"
                                helper="URL to your Hostly instance. If it is empty that means you do not have a FQDN set for your Hostly instance." />
