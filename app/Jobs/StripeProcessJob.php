        ...
        try {
            ...
            switch ($type) {
                ...
                case 'invoice.payment_failed':
                    ...
                    $subscription = Subscription::where('stripe_customer_id', $customerId)->first();
                    if (! $subscription) {
                        send_internal_notification('invoice.payment_failed failed but no subscription found in Hostly for customer: '.$customerId);
                        throw new \RuntimeException("No subscription found for customer: {$customerId}");
                    }
                    $team = data_get($subscription, 'team');
                    if (! $team) {
                        send_internal_notification('invoice.payment_failed failed but no team found in Hostly for customer: '.$customerId);
                        throw new \RuntimeException("No team found in Hostly for customer: {$customerId}");
                    }
                ...
                case 'payment_intent.payment_failed':
                    ...
                    $subscription = Subscription::where('stripe_customer_id', $customerId)->first();
                    if (! $subscription) {
                        send_internal_notification('payment_intent.payment_failed, no subscription found in Hostly for customer: '.$customerId);
                        throw new \RuntimeException("No subscription found in Hostly for customer: {$customerId}");
                    }
                    if ($subscription->stripe_invoice_paid) {
