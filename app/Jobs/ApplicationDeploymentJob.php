...
class ApplicationDeploymentJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    private ?string $hostly_variables = null;
    ...
    private function deploy_docker_compose_buildpack()
    {
        ...
        if ($this->docker_compose_custom_build_command) {
            ...
            $command = "{$this->hostly_variables} docker compose";
        ...
        if ($this->server->isSwarm()) {
            ...
            $this->execute_remote_command([
                ...
                "docker network connect {$networkId} hostly-proxy >/dev/null 2>&1 || true",
                'hidden' => true,
                'ignore_errors' => true,
            ]);
        }
    ...
    private function save_environment_variables()
    {
        ...
        if ($this->pull_request_id !== 0) {
            ...
            if ($this->application->environment_variables_preview->where('key', 'HOSTLY_FQDN')->isEmpty()) {
                $envs->push("HOSTLY_FQDN={$this->preview->fqdn}");
                $envs->push("HOSTLY_DOMAIN_URL={$this->preview->fqdn}");
            }
            if ($this->application->environment_variables_preview->where('key', 'HOSTLY_URL')->isEmpty()) {
                $url = str($this->preview->fqdn)->replace('http://', '')->replace('https://', '');
                $envs->push("HOSTLY_URL={$url}");
                $envs->push("HOSTLY_DOMAIN_FQDN={$url}");
            }
            if ($this->application->build_pack !== 'dockercompose' || $this->application->compose_parsing_version === '1' || $this->application->compose_parsing_version === '2') {
                if ($this->application->environment_variables_preview->where('key', 'HOSTLY_BRANCH')->isEmpty()) {
                    $envs->push("HOSTLY_BRANCH=\"{$local_branch}\"");
                }
                if ($this->application->environment_variables_preview->where('key', 'HOSTLY_CONTAINER_NAME')->isEmpty()) {
                    $envs->push("HOSTLY_CONTAINER_NAME=\"{$this->container_name}\"");
                }
            }

            add_hostly_default_environment_variables($this->application, $envs, $this->application->environment_variables_preview);
            ...
            if ($this->application->environment_variables->where('key', 'HOSTLY_FQDN')->isEmpty()) {
                if ((int) $this->application->compose_parsing_version >= 3) {
                    $envs->push("HOSTLY_URL={$this->application->fqdn}");
                } else {
                    $envs->push("HOSTLY_FQDN={$this->application->fqdn}");
                }
            }
            if ($this->application->environment_variables->where('key', 'HOSTLY_URL')->isEmpty()) {
                $url = str($this->application->fqdn)->replace('http://', '')->replace('https://', '');
                if ((int) $this->application->compose_parsing_version >= 3) {
                    $envs->push("HOSTLY_FQDN={$url}");
                } else {
                    $envs->push("HOSTLY_URL={$url}");
                }
            }
            if ($this->application->build_pack !== 'dockercompose' || $this->application->compose_parsing_version === '1' || $this->application->compose_parsing_version === '2') {
                if ($this->application->environment_variables->where('key', 'HOSTLY_BRANCH')->isEmpty()) {
                    $envs->push("HOSTLY_BRANCH=\"{$local_branch}\"");
                }
                if ($this->application->environment_variables->where('key', 'HOSTLY_CONTAINER_NAME')->isEmpty()) {
                    $envs->push("HOSTLY_CONTAINER_NAME=\"{$this->container_name}\"");
                }
            }

            add_hostly_default_environment_variables($this->application, $envs, $this->application->environment_variables);
    ...
    private function prepare_builder_image()
    {
        ...
        $helperImage = config('constants.hostly.helper_image');
    ...
    private function set_hostly_variables()
    {
        $this->hostly_variables = "SOURCE_COMMIT={$this->commit} ";
        if ($this->pull_request_id === 0) {
        ...
        if (isset($fqdn)) {
            $this->hostly_variables .= "HOSTLY_FQDN={$fqdn} ";
            $url = str($fqdn)->replace('http://', '')->replace('https://', '');
            $this->hostly_variables .= "HOSTLY_URL={$url} ";
        }
        if (isset($this->application->git_branch)) {
            $this->hostly_variables .= "HOSTLY_BRANCH={$this->application->git_branch} ";
        }
    }
    ...
    private function generate_compose_file()
    {
        ...
        if (data_get($this->application, 'custom_labels')) {
            ...
            $labels = $labels->filter(function ($value, $key) {
                return ! Str::startsWith($value, 'hostly.');
            });
    ...
    {
        // Add Hostly related variables to the build args
        $this->environment_variables->filter(function ($key, $value) {
            return str($key)->startsWith('HOSTLY_');
...
LABEL hostly.deploymentId={$this->deployment_uuid}
COPY . .
    ...
    private function stop_running_container(bool $force = false)
    {
        ...
        if ($this->newVersionIsHealthy || $force) {
            ...
            if ($this->application->dockerfile || $this->application->build_pack === 'dockerfile' || $this->application->build_pack === 'dockerimage') {
                $this->application_deployment_queue->addLogEntry('----------------------------------------');
                $this->application_deployment_queue->addLogEntry("WARNING: Dockerfile or Docker Image based deployment detected. The healthcheck needs a curl or wget command to check the health of the application. Please make sure that it is available in the image or turn off healthcheck on Hostly's UI.");
                $this->application_deployment_queue->addLogEntry('----------------------------------------');
            }
    ...
    private function start_by_compose_file()
    {
        if ($this->application->build_pack === 'dockerimage') {
            ...
            $this->execute_remote_command(
                ...
                [executeInDocker($this->deployment_uuid, "{$this->hostly_variables} docker compose --project-name {$this->application->uuid} --project-directory {$this->workdir} up --build -d"), 'hidden' => true],
            );
        } else {
            if ($this->use_build_server) {
                $this->execute_remote_command(
                    ["{$this->hostly_variables} docker compose --project-name {$this->application->uuid} --project-directory {$this->configuration_dir} -f {$this->configuration_dir}{$this->docker_compose_location} up --build -d", 'hidden' => true],
                );
            } else {
                $this->execute_remote_command(
                    [executeInDocker($this->deployment_uuid, "{$this->hostly_variables} docker compose --project-name {$this->application->uuid} --project-directory {$this->workdir} -f {$this->workdir}{$this->docker_compose_location} up --build -d"), 'hidden' => true],
                );
            }
        }
        $this->application_deployment_queue->addLogEntry('New container started.');
