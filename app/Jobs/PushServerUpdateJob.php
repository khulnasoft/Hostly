...
class PushServerUpdateJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    public function handle()
    {
        ...
        foreach ($this->containers as $container) {
            ...
            $hostly_managed = $labels->has('hostly.managed');
            if ($hostly_managed) {
                $name = data_get($container, 'name');
                if ($name === 'hostly-log-drain' && $this->isRunning($containerStatus)) {
                    $this->foundLogDrainContainer = true;
                }
                if ($labels->has('hostly.applicationId')) {
                    $applicationId = $labels->get('hostly.applicationId');
                    $pullRequestId = data_get($labels, 'hostly.pullRequestId', '0');
                    try {
                        if ($pullRequestId === '0') {
                ...
                } elseif ($labels->has('hostly.serviceId')) {
                    $serviceId = $labels->get('hostly.serviceId');
                    $subType = $labels->get('hostly.service.subType');
                    $subId = $labels->get('hostly.service.subId');
                    ...
                    $type = $labels->get('hostly.type');
                    if ($name === 'hostly-proxy' && $this->isRunning($containerStatus)) {
