...
class Init extends Command
{
    ...
    public function handle()
    {
        ...
        $this->restore_hostly_db_backup();
        $this->update_user_emails();
        //
        $this->update_traefik_labels();
        if (! isCloud() || $this->option('force-cloud')) {
            $this->cleanup_unused_network_from_hostly_proxy();
        }
        if (isCloud()) {
        ...
        if (! isCloud()) {
            ...
            if (! is_null(config('constants.hostly.autoupdate', null))) {
                if (config('constants.hostly.autoupdate') == true) {
    ...
    private function cleanup_unnecessary_dynamic_proxy_configuration()
    {
        foreach ($this->servers as $server) {
            try {
                ...
                $file = $server->proxyPath().'/dynamic/hostly.yaml';
    ...
    private function cleanup_unused_network_from_hostly_proxy()
    {
        foreach ($this->servers as $server) {
            if (! $server->isFunctional()) {
                continue;
            ...
            try {
                ...
                foreach ($removeNetworks as $network) {
                    ...
                    if (empty($out)) {
                        $commands->push("docker network disconnect $network hostly-proxy >/dev/null 2>&1 || true");
                        $commands->push("docker network rm $network >/dev/null 2>&1 || true");
                    } else {
                        $data = collect(json_decode($out, true));
                        if ($data->count() === 1) {
                            // If only hostly-proxy itself is connected to that network (it should not be possible, but who knows)
                            $isHostlyProxyItself = data_get($data->first(), 'Name') === 'hostly-proxy';
                            if ($isHostlyProxyItself) {
                                $commands->push("docker network disconnect $network hostly-proxy >/dev/null 2>&1 || true");
                                $commands->push("docker network rm $network >/dev/null 2>&1 || true");
                            }
                        }
                ...
                if ($commands->isNotEmpty()) {
                    echo "Cleaning up unused networks from hostly proxy\n";
                    remote_process(command: $commands, type: ActivityTypes::INLINE->value, server: $server, ignore_errors: false);
                }
            } catch (\Throwable $e) {
                echo "Error in cleaning up unused networks from hostly proxy: {$e->getMessage()}\n";
            }
        }
    }

    private function restore_hostly_db_backup()
    {
        if (version_compare('4.0.0-beta.179', config('constants.hostly.version'), '<=')) {
            try {
                $database = StandalonePostgresql::withTrashed()->find(0);
                if ($database && $database->trashed()) {
                    echo "Restoring hostly db backup\n";
                    $database->restore();
                ...
                echo "Error in restoring hostly db backup: {$e->getMessage()}\n";
            }
        }
    }
    ...
    {
        $id = config('app.id');
        $version = config('constants.hostly.version');
        $settings = instanceSettings();
    ...
    {
        if (version_compare('4.0.0-beta.298', config('constants.hostly.version'), '<=')) {
