...
respond 404
                ...
                $conf =
                    "# This file is automatically generated by Hostly.\n".
    ...
    public function setupDynamicProxyConfiguration()
    {
        ...
        if ($this->proxyType() === ProxyTypes::TRAEFIK->value) {
            $file = "$dynamic_config_path/hostly.yaml";
            if (empty($settings->fqdn) || (isCloud() && $this->id !== 0) || ! $this->isLocalhost()) {
                ...
                if ($schema === 'https') {
                    $traefik_dynamic_conf['http']['routers']['hostly-http']['middlewares'] = [
                        0 => 'redirect-to-https',
                    ];

                    $traefik_dynamic_conf['http']['routers']['hostly-https'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'hostly',
                    ...
                    $traefik_dynamic_conf['http']['routers']['hostly-realtime-wss'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'hostly-realtime',
                    ...
                    $traefik_dynamic_conf['http']['routers']['hostly-terminal-wss'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'hostly-terminal',
            ...
            $file = "$dynamic_config_path/hostly.caddy";
            if (empty($settings->fqdn) || (isCloud() && $this->id !== 0) || ! $this->isLocalhost()) {
                instant_remote_process([
                    "rm -f $file",
                ], $this);
...
$schema://$host {
    handle /app/* {
        reverse_proxy hostly-realtime:6001
    }
    handle /terminal/ws {
        reverse_proxy hostly-realtime:6002
    }
    reverse_proxy hostly:80
}";
    ...
    public function reloadCaddy()
    {
        return instant_remote_process([
            'docker exec hostly-proxy caddy reload --config /config/caddy/Caddyfile.autosave',
        ], $this);
    }

    public function proxyPath()
    {
        $base_path = config('constants.hostly.base_config_path');
        ...
        if ($proxyType === ProxyTypes::TRAEFIK->value) {
            ...
            if (isDev()) {
                $proxy_path = '/var/lib/docker/volumes/hostly_dev_hostly_data/_data/proxy/caddy';
            } else {
                $proxy_path = $proxy_path.'/caddy';
            }
        } elseif ($proxyType === ProxyTypes::NGINX->value) {
            if (isDev()) {
                $proxy_path = '/var/lib/docker/volumes/hostly_dev_hostly_data/_data/proxy/nginx';
            } else {
    ...
    public function getCpuMetrics(int $mins = 5)
    {
        if ($this->isMetricsEnabled()) {
            ...
            $cpu = instant_remote_process(["docker exec hostly-sentinel sh -c 'curl -H \"Authorization: Bearer {$this->settings->sentinel_token}\" http://localhost:8888/api/cpu/history?from=$from'"], $this, false);
    ...
    public function getMemoryMetrics(int $mins = 5)
    {
        if ($this->isMetricsEnabled()) {
            $from = now()->subMinutes($mins)->toIso8601ZuluString();
            $memory = instant_remote_process(["docker exec hostly-sentinel sh -c 'curl -H \"Authorization: Bearer {$this->settings->sentinel_token}\" http://localhost:8888/api/memory/history?from=$from'"], $this, false);
    ...
    {
        $sentinel_found = instant_remote_process(['docker inspect hostly-sentinel'], $this, false);
        $sentinel_found = json_decode($sentinel_found, true);
        $status = data_get($sentinel_found, '0.State.Status', 'exited');
        if ($status === 'running') {
            $containers = instant_remote_process(['docker exec hostly-sentinel sh -c "curl http://127.0.0.1:8888/api/containers"'], $this, false);
    ...
    public function loadUnmanagedContainers(): Collection
    {
        if ($this->isFunctional()) {
            ...
            $containers = $containers->map(function ($container) {
                ...
                if (! str($labels)->contains('hostly.managed')) {
                    return $container;
                }
    ...
    public function databases()
    {
        return $this->destinations()->map(function ($standaloneDocker) {
            ...
            return data_get($item, 'name') !== 'hostly-db';
        });
    }
    ...
    public function validateDockerEngine($throwError = false)
    {
        ...
        $this->settings->save();
        $this->validateHostlyNetwork(isSwarm: false, isBuildServer: $this->settings->is_build_server);
    ...
    public function validateDockerSwarm()
    {
        ...
        $this->settings->is_usable = true;
        $this->settings->save();
        $this->validateHostlyNetwork(isSwarm: true);
    ...
    public function validateHostlyNetwork($isSwarm = false, $isBuildServer = false)
    {
        if ($isBuildServer) {
            return;
        }
        if ($isSwarm) {
            return instant_remote_process(['docker network create --attachable --driver overlay hostly-overlay >/dev/null 2>&1 || true'], $this, false);
        } else {
            return instant_remote_process(['docker network create hostly --attachable >/dev/null 2>&1 || true'], $this, false);
        }
    }
