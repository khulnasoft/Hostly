    ...
    public function handle(Server $server, ?Collection $containers = null, ?Collection $containerReplicates = null)
    {
        ...
        foreach ($this->containers as $container) {
            ...
            if ($applicationId) {
                ...
                $type = data_get($labels, 'hostly.type');

                if ($uuid) {
                    if ($type === 'service') {
                        $database_id = data_get($labels, 'hostly.service.subId');
                        if ($database_id) {
                ...
                if (data_get($container, 'Name') === '/hostly-db') {
                    $foundDatabases[] = 0;
                }
            }
            $serviceLabelId = data_get($labels, 'hostly.serviceId');
            if ($serviceLabelId) {
