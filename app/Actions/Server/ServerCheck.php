...
class ServerCheck
{
                ...
                if ($this->server->proxySet() && ! $this->server->proxy->force_stop) {
                    $foundProxyContainer = $this->containers->filter(function ($value, $key) {
                        if ($this->server->isSwarm()) {
                            return data_get($value, 'Spec.Name') === 'hostly-proxy_traefik';
                        } else {
                            return data_get($value, 'Name') === '/hostly-proxy';
                        }
                    })->first();
                    if (! $foundProxyContainer) {
                        try {
                            ...
                            if ($shouldStart) {
                                ...
                                $this->server->team?->notify(new ContainerRestarted('hostly-proxy', $this->server));
                            }
                        } catch (\Throwable $e) {
                        }
                    } else {
    ...
    private function checkLogDrainContainer()
    {
        $foundLogDrainContainer = $this->containers->filter(function ($value, $key) {
            return data_get($value, 'Name') === '/hostly-log-drain';
        })->first();
    ...
    private function checkContainers()
    {
        foreach ($this->containers as $container) {
            ...
            $managed = data_get($labels, 'hostly.managed');
            if (! $managed) {
                continue;
            }
            $uuid = data_get($labels, 'hostly.name');
            if (! $uuid) {
            ...
            $applicationId = data_get($labels, 'hostly.applicationId');
            $serviceId = data_get($labels, 'hostly.serviceId');
            $databaseId = data_get($labels, 'hostly.databaseId');
            $pullRequestId = data_get($labels, 'hostly.pullRequestId');
            ...
            if ($applicationId) {
                ...
                // Service
                $subType = data_get($labels, 'hostly.service.subType');
                $subId = data_get($labels, 'hostly.service.subId');
                        ...
                        if ($isPublic) {
                            $foundTcpProxy = $this->containers->filter(function ($value, $key) use ($uuid) {
                                if ($this->isSentinel) {
                                    ...
                                    if ($this->server->isSwarm()) {
                                        return data_get($value, 'Spec.Name') === "hostly-proxy_$uuid";
                                    } else {
                                        return data_get($value, 'Name') === "/$uuid-proxy";
                                    }
                                }
