---
title: Docmost
head:
  - tag: "meta"
    attrs:
        property: "og:title"
        content: "How to host Docmost with Hostly"
description: "Here you can find the documentation for hosting Docmost with Hostly."
---

import { Badge } from '@astrojs/starlight/components';

<Badge text="One-click setup." variant="note" size="large" />

## What is Docmost?

Docmost is an open-source collaborative wiki and documentation software.

## Screenshots

![Docmost editor](https://docmost.com/screenshots/editor.png)

![Docmost editor 2](https://docmost.com/screenshots/editor2.png)

![Docmost home](https://docmost.com/screenshots/home.png)

## Links

- [The official website ›](https://www.docmost.com?utm_source=hostly.khulnasoft.com)
- [GitHub ›](https://github.com/docmost/docmost?utm_source=hostly.khulnasoft.com)
