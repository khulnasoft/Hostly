---
title: Unstructured
head:
  - tag: "meta"
    attrs:
        property: "og:title"
        content: "How to host Unstructured with Hostly"
description: "Here you can find the documentation for hosting Unstructured with Hostly."
---

import { Badge } from '@astrojs/starlight/components';

<Badge text="One-click setup." variant="note" size="large" />


## What is Unstructured?

Unstructured provides a platform and tools to ingest and process unstructured documents for Retrieval Augmented Generation (RAG) and model fine-tuning.

## Videos

<iframe
  width="560"
  height="315"
  src="https://www.youtube.com/embed/b2AcxJDXOLs"
  title="YouTube video player"
  frameborder="0"
  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
  allowfullscreen
></iframe>

## Links

- [GitHub ›](https://github.com/Unstructured-IO/unstructured-api?utm_source=hostly.khulnasoft.com)
