---
title: Cloudflared
head:
  - tag: "meta"
    attrs:
        property: "og:title"
        content: "How to host Cloudflared with Hostly"
description: "Here you can find the documentation for hosting Cloudflared with Hostly."
---

import { Badge } from '@astrojs/starlight/components';

<Badge text="One-click setup." variant="note" size="large" />

![Cloudflare](https://avatars.githubusercontent.com/u/314135?s=200&v=4)

## What is Cloudflared?

Cloudflare Tunnel is tunneling software that lets you quickly secure and encrypt application traffic to any type of infrastructure, so you can hide your web server IP addresses, block direct attacks, and get back to delivering great applications.

## Links

- [The official website ›](https://www.cloudflare.com/products/tunnel?utm_source=hostly.khulnasoft.com)
- [GitHub ›](https://github.com/cloudflare/cloudflared?utm_source=hostly.khulnasoft.com)
