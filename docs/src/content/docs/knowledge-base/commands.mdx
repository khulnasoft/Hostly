...
head:
  - tag: "meta"
    attrs:
        property: "og:title"
        content: "What commands are available to manage my Hostly server"
description: A list of commands that you can use to manage your Hostly server.
---
...
```bash
docker exec -ti hostly bash -c "php artisan root:reset-password"
```
...
```bash
docker exec -ti hostly bash -c "php artisan root:change-email"
```
...
```bash
docker exec -ti hostly bash -c "php artisan services:delete"
```
