...
async function main() {
	...
	const isAutoUpdateEnabled = process.env['HOSTLY_AUTO_UPDATE'] === 'true';
	await prisma.setting.update({
		where: {
			id: '0'
		},
...
async function reEncryptSecrets() {
	...
	const image = await execaCommand("docker inspect hostly --format '{{ .Config.Image }}'", {
		shell: true
	});
	...
	await execaCommand('env | grep "^HOSTLY" | sort > .env', {
		shell: true
	});
	const secretOld = process.env['HOSTLY_SECRET_KEY'];
	let secretNew = process.env['HOSTLY_SECRET_KEY_BETTER'];
	if (!secretNew) {
		console.log('No HOSTLY_SECRET_KEY_BETTER found... Generating new one...');
	...
	if (secretOld !== secretNew) {
		...
		console.log(
			'Secrets (HOSTLY_SECRET_KEY & HOSTLY_SECRET_KEY_BETTER) are different, so re-encrypting everything...'
		);
		await execaCommand(`sed -i '/HOSTLY_SECRET_KEY=/d' .env`, { shell: true });
		await execaCommand(`sed -i '/HOSTLY_SECRET_KEY_BETTER=/d' .env`, { shell: true });
		await execaCommand(`echo "HOSTLY_SECRET_KEY=${secretNew}" >> .env`, { shell: true });
		await execaCommand('echo "HOSTLY_SECRET_KEY_BETTER=' + secretNew + '" >> .env ', {
			shell: true
		});
		await execaCommand(`echo "HOSTLY_SECRET_KEY_OLD_${date}=${secretOld}" >> .env`, {
			shell: true
		});
		const transactions = [];
