...
declare module 'fastify' {
	interface FastifyInstance {
		config: {
			HOSTLY_APP_ID: string;
			HOSTLY_SECRET_KEY: string;
			HOSTLY_SECRET_KEY_BETTER: string | null;
			HOSTLY_DATABASE_URL: string;
			HOSTLY_IS_ON: string;
			HOSTLY_WHITE_LABELED: string;
			HOSTLY_WHITE_LABELED_ICON: string | null;
			HOSTLY_AUTO_UPDATE: string;
		};
	}
}
...
(async () => {
	...
	const schema = {
		type: 'object',
		required: ['HOSTLY_SECRET_KEY', 'HOSTLY_DATABASE_URL', 'HOSTLY_IS_ON'],
		properties: {
			HOSTLY_APP_ID: {
				type: 'string'
			},
			...
			HOSTLY_WHITE_LABELED: {
				type: 'string',
				default: 'false'
			},
			HOSTLY_WHITE_LABELED_ICON: {
				type: 'string',
				default: null
			},
			HOSTLY_AUTO_UPDATE: {
	...
	try {
		...
		console.log(`Hostly's API is listening on ${host}:${port}`);
		...
		// Cleanup stucked containers (not defined in Hostly, but still running and managed by Hostly)
		setInterval(async () => {
...
async function getTagsTemplates() {
	...
	try {
		if (isDev) {
			...
			const tags = await got.get('https://get.coollabs.io/hostly/service-tags.json').text();
			const response = await got
				.get('https://get.coollabs.io/hostly/service-templates.yaml')
				.text();
...
async function initServer() {
	const appId = process.env['HOSTLY_APP_ID'];
	...
	try {
		...
		await executeCommand({ command: `docker network create --attachable hostly` });
	} catch (error) { }
	try {
	...
	try {
		...
		for (const destination of destinationDockers) {
			...
			const { stdout: containers } = await executeCommand({
				...
				command: `docker container ps -a --filter "label=hostly.managed=true" --format '{{ .Names}}'`
			});
			if (containers) {
...
async function autoUpdater() {
	try {
		...
		const { hostly } = await got
			.get('https://get.coollabs.io/versions.json', {
				searchParams: {
					appId: process.env['HOSTLY_APP_ID'] || undefined,
					version: currentVersion
				}
			})
			.json();
		const latestVersion = hostly.main.version;
			...
			if (activeCount === 0) {
				if (!isDev) {
					...
					if (isAutoUpdateEnabled) {
						let image = `ghcr.io/khulnasoft/hostly:${latestVersion}`;
						try {
							await executeCommand({ command: `docker pull ${image}` });
						} catch (error) {
							image = `khulnasoft/hostly:${latestVersion}`;
							await executeCommand({ command: `docker pull ${image}` });
						}

						await executeCommand({ shell: true, command: `ls .env || env | grep "^HOSTLY" | sort > .env` });
						await executeCommand({
							command: `sed -i '/HOSTLY_AUTO_UPDATE=/cHOSTLY_AUTO_UPDATE=${isAutoUpdateEnabled}' .env`
						});
						await executeCommand({
							shell: true,
							command: `docker run --rm -tid --env-file .env -v /var/run/docker.sock:/var/run/docker.sock -v hostly-db ${image} /bin/sh -c "env | grep "^HOSTLY" | sort > .env && echo 'TAG=${latestVersion}' >> .env && docker stop -t 0 hostly hostly-fluentbit && docker rm hostly hostly-fluentbit && docker compose pull && docker compose up -d --force-recreate"`
						});
					}
				} else {
	...
	try {
		if (!isDev) {
			...
			const { id } = await prisma.destinationDocker.findFirst({
				where: { engine, network: 'hostly' }
			});
			const { found } = await checkContainer({
				dockerId: id,
				container: 'hostly-fluentbit',
				remove: true
			});
			if (!found) {
				await executeCommand({ shell: true, command: `env | grep '^HOSTLY' > .env` });
...
async function checkProxies() {
	try {
		...
		const localDocker = await prisma.destinationDocker.findFirst({
			where: { engine, network: 'hostly', isHostlyProxyUsed: true }
		});
		if (localDocker) {
		...
		for (const database of databasesWithPublicPort) {
			...
			if (destinationDockerId && destinationDocker.isHostlyProxyUsed) {
...
async function copySSLCertificates() {
	try {
		...
		const destinations = await prisma.destinationDocker.findMany({
			where: { isHostlyProxyUsed: true, teams: { some: { id: { in: [...teamIds] } } } }
		});
...
async function copyRemoteCertificates(id: string, dockerId: string, remoteIpAddress: string) {
	try {
		...
		await executeCommand({
			...
			shell: true,
			dockerId,
			command: `docker exec hostly-proxy sh -c 'test -d /etc/traefik/acme/custom/ || mkdir -p /etc/traefik/acme/custom/'`
		});
		await executeCommand({
			sshCommand: true,
			dockerId,
			command: `docker cp /tmp/${id}-key.pem hostly-proxy:/etc/traefik/acme/custom/`
		});
		await executeCommand({
			sshCommand: true,
			dockerId,
			command: `docker cp /tmp/${id}-cert.pem hostly-proxy:/etc/traefik/acme/custom/`
		});
	} catch (error) {
		console.log({ error });
	}
