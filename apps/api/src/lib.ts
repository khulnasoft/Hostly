...
async function wordpress(service: any, template: any) {
    ...
    if (ownMysql) {
        secrets = [
            ...
            ftpPassword && `HOSTLY_FTP_PASSWORD@@@${ftpPassword}`,
            ftpHostKeyPrivate && `HOSTLY_FTP_HOST_KEY_PRIVATE@@@${ftpHostKeyPrivate}`,
            ftpHostKey && `HOSTLY_FTP_HOST_KEY@@@${ftpHostKey}`,
        ]
        settings = [
        ...
        settings = [
            ...
            `HOSTLY_OWN_DB@@@${ownMysql}`,
            `HOSTLY_FTP_ENABLED@@@${ftpEnabled}`,
            `HOSTLY_FTP_USER@@@${ftpUser}`,
            `HOSTLY_FTP_PUBLIC_PORT@@@${ftpPublicPort}`,
        ]
    }
    ...
    for (const setting of settings) {
        try {
            ...
            if (name === 'MINIO_SERVER_URL') {
                name = 'hostly_fqdn_minio_console'
            }
            if (!value || value === 'null') {
                continue;
            }
