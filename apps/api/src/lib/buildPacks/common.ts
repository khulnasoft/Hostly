...
export const saveBuildLog = async ({
	...
	const fluentBitUrl = isDev
		? process.env.HOSTLY_CONTAINER_DEV === 'true'
			? 'http://hostly-fluentbit:24224'
			: 'http://localhost:24224'
		: 'http://hostly-fluentbit:24224';

	if (isDev && !process.env.HOSTLY_CONTAINER_DEV) {
...
export function makeLabelForSimpleDockerfile({ applicationId, port, type }) {
	return [
		...
		`hostly.applicationId=${applicationId}`,
		`hostly.type=standalone-application`
	];
}
...
export async function buildCacheImageWithNode(data, imageForBuild) {
	...
	Dockerfile.push(`LABEL hostly.buildId=${buildId}`);
	if (secrets.length > 0) {
