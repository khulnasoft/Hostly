...
export const proxyPort = process.env.HOSTLY_PROXY_PORT;
export const proxySecurePort = process.env.HOSTLY_PROXY_SECURE_PORT;
...
const mainTraefikEndpoint = isDev
	...
	: 'http://hostly:3000/webhooks/traefik/main.json';

const otherTraefikEndpoint = isDev
	? `${getAPIUrl()}/webhooks/traefik/other.json`
	: 'http://hostly:3000/webhooks/traefik/other.json';
...
export const getSecretKey = () => {
	if (process.env['HOSTLY_SECRET_KEY_BETTER']) {
		return process.env['HOSTLY_SECRET_KEY_BETTER'];
	}
	return process.env['HOSTLY_SECRET_KEY'];
};
...
export async function isDomainConfigured({
	...
	const hostlyFqdn = await prisma.setting.findFirst({
		where: {
			OR: [
				{ fqdn: { endsWith: `//${nakedDomain}` } },
				{ fqdn: { endsWith: `//www.${nakedDomain}` } }
	...
	return !!(foundApp || foundService || hostlyFqdn);
}
...
export async function createRemoteEngineConfiguration(id: string) {
	...
	config.append({
		...
		ControlMaster: 'auto',
		ControlPath: `${homedir}/.ssh/hostly-${remoteIpAddress}-%r@%h:%p`,
		ControlPersist: '10m'
	});
...
export async function startTraefikProxy(id: string): Promise<void> {
	...
	const { found } = await checkContainer({
		dockerId: id,
		container: 'hostly-proxy',
		remove: true
	});
	const { id: settingsId, ipv4, ipv6 } = await listSettings();

	if (!found) {
		const { stdout: hostlyNetwork } = await executeCommand({
			dockerId: id,
			command: `docker network ls --filter 'name=hostly-infra' --no-trunc --format "{{json .}}"`
		});

		if (!hostlyNetwork) {
			await executeCommand({
				dockerId: id,
				command: `docker network create --attachable hostly-infra`
			});
		}
		...
		await executeCommand({
			...
			-v hostly-traefik-letsencrypt:/etc/traefik/acme \
			-v /var/run/docker.sock:/var/run/docker.sock \
			--network hostly-infra \
...
export async function configureNetworkTraefikProxy(destination: any): Promise<void> {
	...
	const { stdout: networks } = await executeCommand({
		dockerId: id,
		command: `docker ps -a --filter name=hostly-proxy --format '{{json .Networks}}'`
	});
	const configuredNetworks = networks.replace(/"/g, '').replace('\n', '').split(',');
	if (!configuredNetworks.includes(destination.network)) {
		await executeCommand({
			dockerId: destination.id,
			command: `docker network connect ${destination.network} hostly-proxy`
		});
	}
}
...
export async function stopTraefikProxy(
	...
	const { found } = await checkContainer({ dockerId: id, container: 'hostly-proxy' });
	await prisma.destinationDocker.update({
		where: { id },
		data: { isHostlyProxyUsed: false }
	});
	try {
		if (found) {
			await executeCommand({
				dockerId: id,
				command: `docker stop -t 0 hostly-proxy && docker rm hostly-proxy`,
				shell: true
			});
		}
	} catch (error) {
...
export async function makeLabelForStandaloneDatabase({ id, image, volume }) {
	...
	return [
		'hostly.managed=true',
		`hostly.version=${version}`,
		`hostly.type=standalone-database`,
		`hostly.name=${database.name}`,
		`hostly.configuration=${base64Encode(
			JSON.stringify({
				version,
				image,
				volume,
		...
		const interval = setInterval(async () => {
			try {
				...
				const { stdout: buildContainers } = await executeCommand({
					dockerId,
					command: `docker container ls --filter "label=hostly.buildId=${buildId}" --format '{{json .}}'`
				});
				if (buildContainers) {
...
export async function cleanupDockerStorage(dockerId, volumes = false) {
	...
	// Prune hostly managed containers
	try {
		await executeCommand({
			dockerId,
			command: `docker container prune -f --filter "label=hostly.managed=true"`
		});
	} catch (error) { }
