...
export async function newDestination(request: FastifyRequest<NewDestination>, reply: FastifyReply) {
	try {
		...
		let { name, network, engine, isHostlyProxyUsed, remoteIpAddress, remoteUser, remotePort } =
			request.body;
		if (id === 'new') {
			if (engine) {
				...
				await prisma.destinationDocker.create({
					data: { name, teams: { connect: { id: teamId } }, engine, network, isHostlyProxyUsed }
				});
				...
				if (destinations.length > 0) {
					const proxyConfigured = destinations.find(
						(destination) =>
							destination.network !== network && destination.isHostlyProxyUsed === true
					);
					if (proxyConfigured) {
						isHostlyProxyUsed = !!proxyConfigured.isHostlyProxyUsed;
					}
					await prisma.destinationDocker.updateMany({
						where: { engine },
						data: { isHostlyProxyUsed }
					});
				}
				if (isHostlyProxyUsed) {
...
export async function deleteDestination(request: FastifyRequest<OnlyId>) {
	try {
		...
		const { network, remoteVerified, engine, isHostlyProxyUsed } =
			await prisma.destinationDocker.findUnique({ where: { id } });
		if (isHostlyProxyUsed) {
			if (engine || remoteVerified) {
				const { stdout: found } = await executeCommand({
					dockerId: id,
					command: `docker ps -a --filter network=${network} --filter name=hostly-proxy --format '{{.}}'`
				});
				if (found) {
					await executeCommand({
						dockerId: id,
						command: `docker network disconnect ${network} hostly-proxy`
					});
...
export async function saveDestinationSettings(request: FastifyRequest<SaveDestinationSettings>) {
	try {
		const { engine, isHostlyProxyUsed } = request.body;
		await prisma.destinationDocker.updateMany({
			where: { engine },
			data: { isHostlyProxyUsed }
		});
...
export async function verifyRemoteDockerEngineFn(id: string) {
	const { remoteIpAddress, network, isHostlyProxyUsed } = await prisma.destinationDocker.findFirst(
		{ where: { id } }
	);
	...
	try {
		await executeCommand({
			sshCommand: true,
			command: `docker network inspect hostly-infra`,
			dockerId: id
		});
	} catch (error) {
		await executeCommand({
	...
	if (isHostlyProxyUsed) await startTraefikProxy(id);
	let isUpdated = false;
