...
import { checkUpdate, login, showDashboard, update, resetQueue, getCurrentUser, cleanupManually, restartHostly, backup } from './handlers';
...
const root: FastifyPluginAsync = async (fastify): Promise<void> => {
	...
	fastify.post('/internal/restart', {
		onRequest: [fastify.authenticate]
	}, async (request) => await restartHostly(request));
