...
export async function backup(request: FastifyRequest) {
	try {
		...
		if (database) {
			...
			// 	command: `docker pull khulnasoft/backup:latest`,
			// })
			std = await executeCommand({
				dockerId: database.destinationDockerId,
				command: `docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v hostly-local-backup:/app/backups -e CONTAINERS_TO_BACKUP="${backupData}" khulnasoft/backup`
			});
		}
		if (std.stdout) {
			return std.stdout;
...
export async function checkUpdate(request: FastifyRequest) {
	try {
		const { default: got } = await import('got');
		const isStaging =
			request.hostname === 'staging.hostly.io' || request.hostname === 'arm.hostly.io';
...
export async function restartHostly(request: FastifyRequest<any>) {
	try {
		const teamId = request.user.teamId;
		if (teamId === '0') {
			if (!isDev) {
				await executeCommand({ command: `docker restart hostly` });
				return {};
			} else {
				return {};
			}
		...
		throw {
			status: 500,
			message: 'You are not authorized to restart Hostly.'
		};
	} catch ({ status, message }) {
		return errorHandler({ status, message });
	}
					...
					data: {
						...
						teams: {
							create: {
								id: uid,
								name: uniqueName(),
								destinationDocker: { connect: { network: 'hostly' } }
							}
						},
