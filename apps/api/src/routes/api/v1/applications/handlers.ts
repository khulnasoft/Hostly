	...
	try {
		...
		if (application?.destinationDockerId) {
			if (application.buildPack === 'compose') {
				const { stdout: containers } = await executeCommand({
					dockerId: application.destinationDocker.id,
					command: `docker ps -a --filter "label=hostly.applicationId=${id}" --format '{{json .}}'`
				});
				const containersArray = containers.trim().split('\n');
...
export async function loadPreviews(request: FastifyRequest<OnlyId>) {
	try {
		...
		const containers = formatLabelsOnDocker(stdout).filter(
			(container) =>
				container.Labels['hostly.configuration'] &&
				container.Labels['hostly.type'] === 'standalone-application'
		);

		const jsonContainers = containers
			.map((container) =>
				JSON.parse(Buffer.from(container.Labels['hostly.configuration'], 'base64').toString())
			)
			.filter((container) => {
