...
export async function proxyConfiguration(request: FastifyRequest<OnlyId>, remote = false) {
	...
	try {
		...
		const hostlySettings = await prisma.setting.findFirst();
		if (hostlySettings.isTraefikUsed && hostlySettings.proxyDefaultRedirect) {
			...
			traefik.http.middlewares['redirect-regexp'] = {
				redirectregex: {
					regex: '(.*)',
					replacement: hostlySettings.proxyDefaultRedirect,
					permanent: false
				}
			};
		...
		let certificates = await prisma.certificate.findMany({
			where: {
				team: {
					...
					destinationDocker: { some: { remoteEngine: false, isHostlyProxyUsed: true } }
				}
			}
		});
			...
			certificates = await prisma.certificate.findMany({
				where: {
					team: {
						applications: { some: { settings: { isCustomSSL: true } } },
						destinationDocker: {
							some: { id, remoteEngine: true, isHostlyProxyUsed: true, remoteVerified: true }
						}
					}
				}
			});
		...
		if (applications.length > 0) {
			...
			for (const dockerId of dockerIds) {
				const { stdout: container } = await executeCommand({
					dockerId,
					command: `docker container ls --filter 'label=hostly.managed=true' --format '{{ .Names}}'`
				});
				if (container) {
						...
						if (stdout) {
							...
							if (containers.length > 0) {
								for (const container of containers) {
									const previewDomain = `${container.split('-')[1]}${hostlySettings.previewSeparator
										}${domain}`;
		...
		if (!remote) {
			...
			const isHttps = fqdn.startsWith('https://');
			const isWWW = fqdn.includes('www.');
			const id = isDev ? 'host.docker.internal' : 'hostly';
			const container = isDev ? 'host.docker.internal' : 'hostly';
			const port = 3000;
			const pathPrefix = '/';
			const isCustomSSL = false;
			const serviceId = `${id}-${port || 'default'}`;
