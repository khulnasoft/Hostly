<script lang="ts">
	...
	onMount(async () => {
		if (destination.remoteEngine && destination.remoteVerified) {
			...
			if (isRunning === false && destination.isHostlyProxyUsed === true) {
				destination.isHostlyProxyUsed = !destination.isHostlyProxyUsed;
				try {
					await post(`/destinations/${id}/settings`, {
						isHostlyProxyUsed: destination.isHostlyProxyUsed,
			...
			} else if (isRunning === true && destination.isHostlyProxyUsed === false) {
				destination.isHostlyProxyUsed = !destination.isHostlyProxyUsed;
				try {
					await post(`/destinations/${id}/settings`, {
						isHostlyProxyUsed: destination.isHostlyProxyUsed,
	...
	async function changeProxySetting() {
		...
		loading.proxy = true;
		if (!cannotDisable) {
			const isProxyActivated = destination.isHostlyProxyUsed;
			if (isProxyActivated) {
				const sure = confirm(
					`Are you sure you want to ${
						destination.isHostlyProxyUsed ? 'disable' : 'enable'
					} Hostly proxy? It will remove the proxy for all configured networks and all deployments! Nothing will be reachable if you do it!`
				);
				if (!sure) {
					loading.proxy = false;
					return;
			...
			let proxyUsed = !destination.isHostlyProxyUsed;
			try {
				await post(`/destinations/${id}/settings`, {
					isHostlyProxyUsed: proxyUsed,
					engine: destination.engine
				});
				if (isProxyActivated) {
					await stopProxy();
				...
				destination.isHostlyProxyUsed = proxyUsed;
			} catch (error) {
				return errorNotification(error);
			} finally {
				loading.proxy = false;
	...
	async function stopProxy() {
		try {
			...
			return addToast({
				message: $t('destination.hostly_proxy_stopped'),
				type: 'success'
			});
		} catch (error) {
			return errorNotification(error);
	...
	async function startProxy() {
		try {
			...
			return addToast({
				message: $t('destination.hostly_proxy_started'),
				type: 'success'
			});
		} catch (error) {
			return errorNotification(error);
	...
	async function forceRestartProxy() {
		...
		if (sure) {
			try {
				...
				addToast({
					message: $t('destination.hostly_proxy_restarting'),
					type: 'success'
				});
...
<form on:submit|preventDefault={handleSubmit} class="grid grid-flow-row gap-2 py-4">
	...
	<div class="grid grid-cols-2 items-center px-10">
		...
		<CopyPasswordField
			...
			disabled
			name="network"
			placeholder="{$t('forms.default')}: hostly"
	...
	<div class="grid grid-cols-2 items-center px-10">
		<Setting
			...
			bind:setting={destination.isHostlyProxyUsed}
			on:click={changeProxySetting}
			title={$t('destination.use_hostly_proxy')}
			description={`Install & configure a proxy (based on Traefik) on the destination to allow you to access your applications and services without any manual configuration.${
				cannotDisable
					? '<span class="font-bold text-white">You cannot disable this proxy as FQDN is configured for Hostly.</span>'
					: ''
			}`}
		/>
	</div>
