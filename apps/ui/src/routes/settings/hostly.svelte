...
<script lang="ts">
	...
	async function rollback() {
		if (rollbackVersion) {
			const sure = confirm(`Are you sure you want upgrade Hostly to ${rollbackVersion}?`);
			if (sure) {
				try {
	...
	async function restartHostly() {
		const sure = confirm(
			'Are you sure you would like to restart Hostly? Currently running deployments will be stopped and restarted.'
		);
		if (sure) {
...
<div class="mx-auto w-full">
	<form on:submit|preventDefault={handleSubmit}>
		<div class="flex flex-row border-b border-coolgray-500 mb-6">
			<div class="title font-bold pb-3 pr-4">Hostly Settings</div>
			<div class="flex flex-row space-x-2">
				...
				{/if}
				<button
					on:click={restartHostly}
					class:loading={loading.restart}
					class="btn btn-sm btn-error">Restart Hostly</button
				>
			</div>
		</div>
			...
			<div class="grid grid-flow-row gap-2 px-4 pr-5">
				<div class="grid grid-cols-2 items-center">
					...
					<input
						...
						placeholder="{$t('forms.eg')}: https://hostly.io"
					/>
				...
				<div class="grid grid-cols-4 items-center pb-12">
					<div class="col-span-2">
						Upgrade Hostly to a specific version
						<Explainer
							position="dropdown-bottom"
							explanation="You can upgrade to a specific version of Hostly. This will not affect your current running resources, but could cause issues if you downgrade to an older version where the database layout was different..<br><br><a href='https://github.com/khulnasoft/hostly/releases' target='_blank'>See available versions</a>"
						/>
					</div>
					<input
						class="w-full"
				...
				<div class="grid grid-cols-2 items-center">
					<div>
						Custom DNS servers <Explainer
							explanation="You can specify a custom DNS server to verify your domains all over Hostly.<br><br>By default, the OS defined DNS servers are used."
						/>
					</div>
				...
				<div class="grid grid-cols-2 items-center">
					<Setting
						...
						title="API Debugging"
						description="Enable API debugging. This will log all API requests and responses.<br><br>You need to restart the Hostly for this to take effect."
				...
				<div class="grid grid-cols-2 items-center">
					<Setting
						...
						title="Do Not Track"
						description="Do not send error reports to Hostly developers or any telemetry."
