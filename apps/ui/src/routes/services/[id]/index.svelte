...
<script lang="ts">
	...
	async function handleSubmit(e: any) {
		...
		try {
			...
			for (const setting of service.serviceSetting) {
				if (setting.variableName?.startsWith('$$config_hostly_fqdn') && setting.value) {
	...
	onMount(async () => {
		if (browser && window.location.hostname === 'demo.hostly.io' && !service.fqdn) {
			service.fqdn = `http://${cuid()}.demo.hostly.io`;
		}
	});
</script>
			...
			{#each Object.keys(template) as oneService}
				{#each template[oneService].fqdns as fqdn}
					<div class="grid grid-cols-2 items-center py-1">
						...
						<CopyPasswordField
							placeholder="eg: https://hostly.io"
							readonly={isDisabled}
							disabled={isDisabled}
