		...
		{#if $appSession.userId}
			...
			<nav class="nav-main hidden lg:block z-20">
				<div class="flex h-screen w-full flex-col items-center transition-all duration-100">
					...
					<div class="flex flex-col space-y-2 py-2">
						...
						<a
							id="settings"
							href={$appSession.teamId === '0' ? '/settings/hostly' : '/settings/docker'}
						...
						<a
							id="documentation"
							href="https://docs.coollabs.io/hostly-v3/"
							target="_blank"
						...
						>
							<a
								...
								href={`https://github.com/khulnasoft/hostly/releases/tag/v${$appSession.version}`}
			...
			{#if $appSession.whiteLabeled}
				<span class="fixed bottom-0 left-[50px] z-50 m-2 px-4 text-xs text-stone-700"
					>Powered by <a href="https://hostly.io" target="_blank noreferrer">Hostly</a></span
				>
			{/if}
		{/if}
		<div
		...
		>
			<div>
				...
				<div class="prose flex flex-row justify-between space-x-1 w-full items-center pr-3">
					{#if !$appSession.whiteLabeled}
						<h3 class="mb-0 text-white">Hostly</h3>
					{/if}
				</div>
			</div>
			<!-- <LocalePicker /> -->
		...
		<ul class="menu bg-coolgray-200 w-60 p-2 space-y-3 pt-4">
			...
			<li>
				<a
					...
					href={$appSession.teamId === '0' ? '/settings/hostly' : '/settings/ssh'}
