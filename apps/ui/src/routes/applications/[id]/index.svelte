...
<script lang="ts">
	...
	onMount(async () => {
		if (window.location.hostname === 'demo.hostly.io' && !application.fqdn) {
			application.fqdn = `http://${cuid()}.demo.hostly.io`;
			await handleSubmit();
		}
				...
				{#if application.dockerRegistry?.id && application.gitSourceId}
					<div class="grid grid-cols-2 items-center">
						...
						<input
							...
							disabled={isDisabled}
							class="w-full"
							placeholder="e.g. khulnasoft/myimage (tag will be commit sha) or khulnasoft/myimage:tag"
