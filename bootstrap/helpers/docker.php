...
function getCurrentApplicationContainerStatus(Server $server, int $id, ?int $pullRequestId = null, ?bool $includePullrequests = false): Collection
{
    $containers = collect([]);
    if (! $server->isSwarm()) {
        $containers = instant_remote_process(["docker ps -a --filter='label=hostly.applicationId={$id}' --format '{{json .}}' "], $server);
        $containers = format_docker_command_output_to_json($containers);
        $containers = $containers->map(function ($container) use ($pullRequestId, $includePullrequests) {
            $labels = data_get($container, 'Labels');
            if (! str($labels)->contains('hostly.pullRequestId=')) {
                data_set($container, 'Labels', $labels.",hostly.pullRequestId={$pullRequestId}");
            ...
            if (str($labels)->contains("hostly.pullRequestId=$pullRequestId")) {
                return $container;
            }
...
function getCurrentServiceContainerStatus(Server $server, int $id): Collection
{
    $containers = collect([]);
    if (! $server->isSwarm()) {
        $containers = instant_remote_process(["docker ps -a --filter='label=hostly.serviceId={$id}' --format '{{json .}}' "], $server);
        $containers = format_docker_command_output_to_json($containers);
...
function defaultLabels($id, $name, $pull_request_id = 0, string $type = 'application', $subType = null, $subId = null)
{
    ...
    $labels->push('hostly.version='.config('constants.hostly.version'));
    $labels->push('hostly.'.$type.'Id='.$id);
    $labels->push("hostly.type=$type");
    $labels->push('hostly.name='.$name);
    $labels->push('hostly.pullRequestId='.$pull_request_id);
    if ($type === 'service') {
        $subId && $labels->push('hostly.service.subId='.$subId);
        $subType && $labels->push('hostly.service.subType='.$subType);
    }
...
function fqdnLabelsForTraefik(string $uuid, Collection $domains, bool $is_force_https_enabled = false, $onlyPort = null, ?Collection $serviceLabels = null, ?bool $is_gzip_enabled = true, ?bool $is_stripprefix_enabled = true, ?string $service_name = null, bool $generate_unique_uuid = false, ?string $image = null, string $redirect_direction = 'both')
{
    ...
    if ($serviceLabels) {
        $middlewares_from_labels = $serviceLabels->map(function ($item) {
            ...
            if (preg_match('/hostly\.traefik\.middlewares=(.*)/', $item, $matches)) {
