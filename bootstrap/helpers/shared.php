...
function get_latest_version_of_hostly(): string
{
    try {
        $versions = File::get(base_path('versions.json'));
        $versions = json_decode($versions, true);

        return data_get($versions, 'hostly.v4.version');
    } catch (\Throwable $e) {
        ray($e->getMessage());
...
function generateSSHKey(string $type = 'rsa')
{
    if ($type === 'rsa') {
        ...
        return [
            ...
            'public' => $key->getPublicKey()->toString('OpenSSH', ['comment' => 'hostly-generated-ssh-key']),
        ];
    } elseif ($type === 'ed25519') {
...
{
    return ! config('constants.hostly.self_hosted');
}
...
function check_domain_usage(ServiceApplication|Application|null $resource = null, ?string $domain = null)
{
    ...
    if ($resource) {
        ...
        if (data_get($settings, 'fqdn')) {
            ...
            $naked_domain = str($domain)->value();
            if ($domains->contains($naked_domain)) {
                throw new \RuntimeException("Domain $naked_domain is already in use by this Hostly instance.");
            }
        }
    }
}
...
function parseDockerComposeFile(Service|Application $resource, bool $isNew = false, int $pull_request_id = 0, ?int $preview_id = null)
{
    if ($resource->getMorphClass() === \App\Models\Service::class) {
        if ($resource->docker_compose_raw) {
            ...
            $envs_from_hostly = $resource->environment_variables()->get();
            $services = collect($services)->map(function ($service, $serviceName) use ($resource, $envs_from_hostly) {
                $serviceVariables = collect(data_get($service, 'environment', []));
                ...
                $parsedServiceVariables->put('HOSTLY_CONTAINER_NAME', "$serviceName-{$resource->uuid}");

                // TODO: move this in a shared function
                if (! $parsedServiceVariables->has('HOSTLY_APP_NAME')) {
                    $parsedServiceVariables->put('HOSTLY_APP_NAME', "\"{$resource->name}\"");
                }
                if (! $parsedServiceVariables->has('HOSTLY_SERVER_IP')) {
                    $parsedServiceVariables->put('HOSTLY_SERVER_IP', "\"{$resource->destination->server->ip}\"");
                }
                if (! $parsedServiceVariables->has('HOSTLY_ENVIRONMENT_NAME')) {
                    $parsedServiceVariables->put('HOSTLY_ENVIRONMENT_NAME', "\"{$resource->environment->name}\"");
                }
                if (! $parsedServiceVariables->has('HOSTLY_PROJECT_NAME')) {
                    $parsedServiceVariables->put('HOSTLY_PROJECT_NAME', "\"{$resource->project()->name}\"");
                }

                $parsedServiceVariables = $parsedServiceVariables->map(function ($value, $key) use ($envs_from_hostly) {
                    if (! str($value)->startsWith('$')) {
                        $found_env = $envs_from_hostly->where('key', $key)->first();
                        if ($found_env) {
...
function newParser(Application|Service $resource, int $pull_request_id = 0, ?int $preview_id = null): Collection
{
    ...
    foreach ($services as $serviceName => $service) {
        ...
        if ($isService) {
            ...
            // Add Hostly defined environments
    ...
    foreach ($services as $serviceName => $service) {
                ...
                if ($type->value() === 'bind') {
                    if ($source->value() === '/var/run/docker.sock') {
                        ...
                        if (isDev()) {
                            if ((int) $resource->compose_parsing_version >= 4) {
                                if ($isApplication) {
                                    $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/hostly_dev_hostly_data/_data/applications/'.$uuid);
                                } elseif ($isService) {
                                    $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/hostly_dev_hostly_data/_data/services/'.$uuid);
                                }
                            } else {
                                $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/hostly_dev_hostly_data/_data/applications/'.$uuid);
                            }
            ...
            if ($key->value() === $parsedValue->value()) {
                ...
                if ($value->startsWith('$')) {
                    ...
                    if ($originalValue->value() === $value->value()) {
                        // This means the variable does not have a default value, so it needs to be created in Hostly
        ...
        if ($isApplication) {
            ...
            if ($originalResource->environment_variables->where('key', 'HOSTLY_BRANCH')->isEmpty()) {
                $hostlyEnvironments->put('HOSTLY_BRANCH', "\"{$branch}\"");
            }
        }

        // Add HOSTLY_CONTAINER_NAME to environment
        if ($resource->environment_variables->where('key', 'HOSTLY_CONTAINER_NAME')->isEmpty()) {
            $hostlyEnvironments->put('HOSTLY_CONTAINER_NAME', "\"{$containerName}\"");
        }
        ...
        // Add HOSTLY_FQDN & HOSTLY_URL to environment
        if (! $isDatabase && $fqdns instanceof Collection && $fqdns->count() > 0) {
            $hostlyEnvironments->put('HOSTLY_URL', $fqdns->implode(','));

            $urls = $fqdns->map(function ($fqdn) {
                return str($fqdn)->replace('http://', '')->replace('https://', '');
            });
            $hostlyEnvironments->put('HOSTLY_FQDN', $urls->implode(','));
        }
        add_hostly_default_environment_variables($resource, $hostlyEnvironments, $resource->environment_variables);

        if ($environment->count() > 0) {
            $environment = $environment->filter(function ($value, $key) {
                return ! str($key)->startsWith('SERVICE_FQDN_');
            })->map(function ($value, $key) use ($resource) {
                // if value is empty, set it to null so if you set the environment variable in the .env file (Hostly's UI), it will used
        ...
        if ($environment->count() > 0 || $hostlyEnvironments->count() > 0) {
            $payload['environment'] = $environment->merge($hostlyEnvironments);
        }
        if ($logging) {
...
function generate_fluentd_configuration(): array
{
    return [
        ...
        'options' => [
            ...
            'env' => 'HOSTLY_APP_NAME,HOSTLY_PROJECT_NAME,HOSTLY_SERVER_IP,HOSTLY_ENVIRONMENT_NAME',
        ],
    ];
}
 ...
 * - HOSTLY_PROJECT_NAME
 * - HOSTLY_SERVER_IP
 * - HOSTLY_ENVIRONMENT_NAME
 *
 *  Theses variables are added in place to the $where_to_add array.
 */
function add_hostly_default_environment_variables(StandaloneRedis|StandalonePostgresql|StandaloneMongodb|StandaloneMysql|StandaloneMariadb|StandaloneKeydb|StandaloneDragonfly|StandaloneClickhouse|Application|Service $resource, Collection &$where_to_add, ?Collection $where_to_check = null)
{
    // Currently disabled
    return;
    ...
    if ($where_to_check != null && $where_to_check->where('key', 'HOSTLY_APP_NAME')->isEmpty()) {
        if ($isAssociativeArray) {
            $where_to_add->put('HOSTLY_APP_NAME', "\"{$resource->name}\"");
        } else {
            $where_to_add->push("HOSTLY_APP_NAME=\"{$resource->name}\"");
        }
    }
    if ($where_to_check != null && $where_to_check->where('key', 'HOSTLY_SERVER_IP')->isEmpty()) {
        if ($isAssociativeArray) {
            $where_to_add->put('HOSTLY_SERVER_IP', "\"{$ip}\"");
        } else {
            $where_to_add->push("HOSTLY_SERVER_IP=\"{$ip}\"");
        }
    }
    if ($where_to_check != null && $where_to_check->where('key', 'HOSTLY_ENVIRONMENT_NAME')->isEmpty()) {
        if ($isAssociativeArray) {
            $where_to_add->put('HOSTLY_ENVIRONMENT_NAME', "\"{$resource->environment->name}\"");
        } else {
            $where_to_add->push("HOSTLY_ENVIRONMENT_NAME=\"{$resource->environment->name}\"");
        }
    }
    if ($where_to_check != null && $where_to_check->where('key', 'HOSTLY_PROJECT_NAME')->isEmpty()) {
        if ($isAssociativeArray) {
            $where_to_add->put('HOSTLY_PROJECT_NAME', "\"{$resource->project()->name}\"");
        } else {
            $where_to_add->push("HOSTLY_PROJECT_NAME=\"{$resource->project()->name}\"");
        }
    }
}
...
function loadConfigFromGit(string $repository, string $branch, string $base_directory, int $server_id, int $team_id)
{
    ...
    $fileList = collect([".$workdir/hostly.json"]);
    $commands = collect([
        "rm -rf /tmp/{$uuid}",
        "mkdir -p /tmp/{$uuid}",
        "cd /tmp/{$uuid}",
