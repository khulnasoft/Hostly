...
function collectProxyDockerNetworksByServer(Server $server)
{
    ...
    $networks = instant_remote_process(['docker inspect --format="{{json .NetworkSettings.Networks }}" hostly-proxy'], $server, false);
...
function collectDockerNetworksByServer(Server $server)
{
    ...
    if ($server->isSwarm()) {
        if ($networks->count() === 0) {
            $networks = collect(['hostly-overlay']);
            $allNetworks = collect(['hostly-overlay']);
        }
    } else {
        if ($networks->count() === 0) {
            $networks = collect(['hostly']);
            $allNetworks = collect(['hostly']);
        }
    }
...
function connectProxyToNetworks(Server $server)
{
    ...
    if ($server->isSwarm()) {
        $commands = $networks->map(function ($network) {
            return [
                "echo 'Connecting hostly-proxy to $network network...'",
                "docker network ls --format '{{.Name}}' | grep '^$network$' >/dev/null || docker network create --driver overlay --attachable $network >/dev/null",
                "docker network connect $network hostly-proxy >/dev/null 2>&1 || true",
                "echo 'Successfully connected hostly-proxy to $network network.'",
