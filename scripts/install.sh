...
echo -e "Welcome to Hostly Installer!"
echo -e "This script will install everything for you. Sit back and relax."
echo -e "Source code: https://github.com/khulnasoft/hostly/blob/main/scripts/install.sh\n"
...
mkdir -p /data/hostly/{source,ssh,applications,databases,backups,services,proxy,webhooks-during-maintenance,sentinel}
...
INSTALLATION_LOG_WITH_DATE="/data/hostly/source/installation-${DATE}.log"
...
ENV_FILE="/data/hostly/source/.env"
...
if [ "$SSH_DETECTED" = "false" ]; then
    ...
    *)
        ...
        echo -e "Please make sure it is installed and running, otherwise Hostly cannot connect to the host system. \n"
        echo "###############################################################################"
        exit 1
        ;;
    esac
...
if [ "$SSH_PERMIT_ROOT_LOGIN" = "yes" ] || [ "$SSH_PERMIT_ROOT_LOGIN" = "without-password" ] || [ "$SSH_PERMIT_ROOT_LOGIN" = "prohibit-password" ]; then
    ...
    echo "   If you have problems with SSH, please read this: https://hostly.khulnasoft.com/docs/knowledge-base/server/openssh"
fi
...
if [ -x "$(command -v snap)" ]; then
    ...
    if [ "$SNAP_DOCKER_INSTALLED" = "true" ]; then
        ...
        echo "   Please note that Hostly does not support Docker installed via snap."
...
EOL
cat >/etc/docker/daemon.json.hostly <<EOL
{
  "log-driver": "json-file",
  "log-opts": {
    "max-size": "10m",
...
EOL
TEMP_FILE=$(mktemp)
if ! jq -s '.[0] * .[1]' /etc/docker/daemon.json /etc/docker/daemon.json.hostly >"$TEMP_FILE"; then
...
curl -fsSL $CDN/upgrade.sh -o /data/hostly/source/upgrade.sh
...
if [ -f $ENV_FILE ]; then
    ...
    cp /data/hostly/source/.env.production $ENV_FILE-$DATE
...
awk -F '=' '!seen[$1]++' "$ENV_FILE-$DATE" /data/hostly/source/.env.production > $ENV_FILE

if [ "$AUTOUPDATE" = "false" ]; then
    if ! grep -q "AUTOUPDATE=" /data/hostly/source/.env; then
        echo "AUTOUPDATE=false" >>/data/hostly/source/.env
    else
        sed -i "s|AUTOUPDATE=.*|AUTOUPDATE=false|g" /data/hostly/source/.env
    fi
fi
...
set +e
IS_HOSTLY_VOLUME_EXISTS=$(docker volume ls | grep hostly-db | wc -l)
set -e

if [ "$IS_HOSTLY_VOLUME_EXISTS" -eq 0 ]; then
    echo " - Generating SSH key."
    ssh-keygen -t ed25519 -a 100 -f /data/hostly/ssh/keys/id.$CURRENT_USER@host.docker.internal -q -N "" -C hostly
    chown 9999 /data/hostly/ssh/keys/id.$CURRENT_USER@host.docker.internal
    sed -i "/hostly/d" ~/.ssh/authorized_keys
    cat /data/hostly/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub >> ~/.ssh/authorized_keys
    rm -f /data/hostly/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub
fi

chown -R 9999:root /data/hostly
chmod -R 700 /data/hostly

echo -e "9. Installing Hostly ($LATEST_VERSION)"
echo -e " - It could take a while based on your server's performance, network speed, stars, etc."
echo -e " - Please wait."
getAJoke

bash /data/hostly/source/upgrade.sh "${LATEST_VERSION:-latest}" "${LATEST_HELPER_VERSION:-latest}"
echo " - Hostly installed successfully."
rm -f $ENV_FILE-$DATE

echo " - Waiting for 20 seconds for Hostly (database migrations) to be ready."
getAJoke
...
echo -e "You can access Hostly through your Public IP: http://$(curl -4s https://ifconfig.io):8000"
...
echo -e "\nWARNING: It is highly recommended to backup your Environment variables file (/data/hostly/source/.env) to a safe location, outside of this server (e.g. into a Password Manager).\n"
cp /data/hostly/source/.env /data/hostly/source/.env.backup
