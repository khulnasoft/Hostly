...
const verifyClient = async (info, callback) => {
    ...
    try {
        ...
        const response = await axios.post(`http://hostly/terminal/auth`, null, {
            headers: {
...
async function handleCommand(ws, command, userId) {
    ...
    //         Establishes an SSH connection to root@hostly with RequestTTY enabled
...
server.listen(6002, () => {
    console.log('Hostly realtime terminal server listening on port 6002. Let the hacking begin!');
});
