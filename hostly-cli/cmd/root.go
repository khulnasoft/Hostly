...
var rootCmd = &cobra.Command{
	Use:   "hostly",
	Short: "Hostly CLI",
	Long:  `A CLI tool to interact with Hostly API.`,
...
func CheckMinimumVersion(version string) {
	...
	if currentVersion.LessThan(requiredVersion) {
		log.Printf("Minimum required Hostly API version is: %s\n", version)
		log.Print("Please upgrade your Hostly instance for this command.\n")
		os.Exit(0)
	}
}
...
func CheckLatestVersionOfCli() (string, error) {
	...
	url := "https://api.github.com/repos/khulnasoft/hostly/hostly-cli/git/refs/tags"
	req, err := http.NewRequest("GET", url, nil)
	if err != nil {
		return "", err
	}
	...
	if latestVersion != CliVersion {
		fmt.Printf("There is a new version of Hostly CLI available.\nPlease update with 'hostly --update'.\n\n")
	}
...
func initConfig() {
	...
	viper.AddConfigPath(ConfigDir + "/hostly")
	if _, err := os.Stat(ConfigDir + "/hostly"); os.IsNotExist(err) {
		os.MkdirAll(ConfigDir+"/hostly", 0755)
	}
	if err := viper.ReadInConfig(); err != nil {
		if _, ok := err.(viper.ConfigFileNotFoundError); ok {
			log.Println("Config file not found. Creating a new one at", ConfigDir+"/hostly/config.json")
	...
	if data != CliVersion {
		log.Printf("New version of Hostly CLI is available: %s\n", data)
	}
}
